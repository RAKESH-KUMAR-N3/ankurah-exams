...
import { Sparkles, CheckCircle2, Shield, X, ArrowRight, Loader2 } from 'lucide-react';
import { fetchPlans, fetchStudentTypes, createPaymentOrder, verifyPaymentOrder } from '../../lib/api';
...
export default function PlanStore({ user, onPurchaseSuccess }: PlanStoreProps) {
  ...
  const [mockPaymentState, setMockPaymentState] = useState<'idle' | 'processing' | 'success'>('idle');
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([plansRes, stRes]: [any, any]) => {
        ...
        setStudentTypes(stList.map((st: any) => ({ id: st._id || st.id, name: st.name })));
        setLoading(false);
      })
      .catch((err) => {
  ...
  const handleBuyClick = (plan: Plan) => {
    ...
    if (allowed.length > 0) {
      const firstId = typeof allowed[0] === 'string' ? allowed[0] : (allowed[0]._id || allowed[0].id);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {plans.map((plan) => {
          ...
          const isPurchased = (user?.purchasedPlans || []).some((p: any) => {
            if (p.isActive === false) return false;
            const pPlanId = (p.planId?._id || p.planId || '').toString();
            const pExamId = (p.examId?._id || p.examId || '').toString();
            return (pPlanId && pPlanId === targetPlanId) || (pExamId && targetExamId && pExamId === targetExamId);
          ...
          return (
            <div key={plan._id} className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200/60 hover:shadow-md transition-all flex flex-col h-full relative overflow-hidden group">
              ...
              <div className="flex items-center justify-between gap-2 mb-2">
                <h3 className="text-xl font-black text-slate-900 tracking-tight">{courseName}</h3>
              ...
              <div className="text-3xl font-black text-emerald-600 mb-3">₹{plan.price}</div>
              <p className="text-sm text-slate-600 mb-6 flex-grow font-medium leading-relaxed">{plan.description}</p>
          ...
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-6 relative overflow-hidden animate-in fade-in duration-200">
            ...
            <div>
              ...
              <h3 className="text-xl font-black text-slate-900">
                {(typeof groupModalPlan.examId === 'object' && groupModalPlan.examId?.name) || 'Course Plan'}
              </h3>
              <p className="text-slate-500 text-xs font-medium mt-1">
                Select your student group to map your exact study timetable, tests, and materials.
              </p>
            </div>

            {/* Allowed Groups Radio List */}
            {(() => {
              const examObj = typeof groupModalPlan.examId === 'object' ? groupModalPlan.examId : null;
              ...
              return (
                <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                  {allowedGroups.map((st: any) => {
                    ...
                    const stObjName = typeof st === 'object' && st !== null && st.name ? st.name : null;
                    const fallbackName = studentTypes.find(s => s.id === stId)?.name;
            ...
            <button
              ...
              disabled={!selectedGroupId && ((typeof groupModalPlan.examId === 'object' && groupModalPlan.examId?.allowedStudentTypes?.length) || 0) > 0}
            ...
            >
              Proceed to Pay ₹{groupModalPlan.price} <ArrowRight className="w-4 h-4" />
            </button>
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-3xl max-w-sm w-full p-8 text-center shadow-2xl animate-fade-in-up">
            {mockPaymentState === 'processing' ? (
              <div className="flex flex-col items-center">
                ...
                <p className="text-slate-500 text-sm">Please wait while we securely process your payment for {mockPaymentPlan.name}.</p>
              </div>
            ) : (
              <div className="flex flex-col items-center">
                ...
                <p className="text-slate-500 text-sm mb-8">You have successfully subscribed to the <span className="font-semibold text-slate-700">{mockPaymentPlan.name}</span> plan.</p>
                <button 
                  onClick={() => {
                    setMockPaymentState('idle');
                    onPurchaseSuccess();
