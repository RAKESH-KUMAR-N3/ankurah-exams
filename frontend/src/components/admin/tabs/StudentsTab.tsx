    ...
    <div className="space-y-6 text-slate-100 font-sans">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-900/90 p-6 rounded-2xl border border-slate-800 shadow-xl backdrop-blur-md">
        <div>
          ...
          <p className="text-xs text-slate-400 font-medium mt-1">View student groups, enrolled plans, and active subscriptions.</p>
        </div>
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-slate-800/80 border border-slate-700 rounded-xl text-xs font-semibold text-slate-100 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500"
          />
        </div>
      </div>
      ...
      <div className="bg-slate-900/90 rounded-2xl border border-slate-800 shadow-xl overflow-hidden backdrop-blur-md">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className="bg-slate-800/80 border-b border-slate-700/80 text-slate-300 font-extrabold uppercase tracking-wider">
            ...
            <tbody className="divide-y divide-slate-800/80 font-semibold text-slate-200">
              {filteredStudents.length === 0 ? (
                ...
                filteredStudents.map((student: any) => {
                  ...
                  return (
                    <tr key={student.uid || student._id} className="hover:bg-slate-800/50 transition-colors">
                      <td className="p-4">
                        <div className="font-extrabold text-white text-sm">{student.name}</div>
                        <div className="text-slate-400 text-[11px] font-medium">{student.email}</div>
                      </td>
                      <td className="p-4">
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-slate-800 text-slate-200 font-bold rounded-lg border border-slate-700">
                      ...
                      <td className="p-4">
                        <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-emerald-500/20 text-emerald-400 font-black text-[10px] uppercase rounded-full border border-emerald-500/30">
