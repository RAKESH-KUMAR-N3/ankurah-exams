...
export const createExam = asyncHandler(async (req: Request, res: Response) => {
  const { id, name, description, type, price, allowedStudentTypes } = req.body;
  ...
  if (price !== undefined && price !== null && price !== '') {
    await Plan.create({
      examId: exam._id,
      name: type === 'entrance' ? 'Yearly Plan' : `${name.trim()} Plan`,
...
export const updateExam = asyncHandler(async (req: Request, res: Response) => {
  const { name, type, categoryId, allowedStudentTypes, price } = req.body;
  ...
  if (exam) {
    ...
    if (allowedStudentTypes) exam.allowedStudentTypes = allowedStudentTypes;
